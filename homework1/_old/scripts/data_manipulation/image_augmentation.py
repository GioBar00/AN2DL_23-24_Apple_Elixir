...
DATASET = np.load("./res/npz/public_data_clean.npz", allow_pickle=True)
...
num_img = 10
fig, axes = plt.subplots(num_img, num_img, figsize=(96,96))

# Iterate through the selected number of images
for i in range(num_img):
    for j in range(num_img):
        index = i*num_img + j
        img_norm = gen_X_train[index]/255.0                  # normalize the values from 0..255 -> 0..1
        axes[i,j].imshow(np.clip(img_norm, 0,1))               # Show the image
        axes[i,j].set_title(f'{gen_y_train[index]}, {index}')  
